...
Version 1.0.0 was signed for unlisted distribution. Upload `zip/Firefox v1.2.0.zip` with **On this site** selected for the public submission. Deleting a Mozilla version does not allow reuse of its version number. Both browser packages now use 1.2.0; see [the shared release workflow](RELEASING.md). The package requires Firefox 142 or newer. Android support is not declared or tested.
...
The Firefox ZIP is written to `zip/Firefox v1.2.0.zip`. Validation uses Mozilla's pinned `addons-linter`; all files are scanned, errors fail the command, and warnings remain visible. The full report is `zip/firefox-validation.json`. ZIP timestamps can differ between builds.
